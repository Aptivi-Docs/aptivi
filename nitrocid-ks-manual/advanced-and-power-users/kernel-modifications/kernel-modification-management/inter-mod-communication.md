...
* The current user doesn't have the `IntermodCommunication` permission for privileged members in another mod.
{% endhint %}
...
<summary>Protecting a member</summary>

You can protect a method, a field, or a property in the public API of a kernel mod by adding the `CommunicationPrivilegedAttribute` attribute so that users without the `InteraddonCommunication` permission can't access it.

This is needed in cases where you're implementing a command in one mod that calls a function, a property, or a field in another mod where they are sensitive areas, such as those related to privileged kernel features like power management.
