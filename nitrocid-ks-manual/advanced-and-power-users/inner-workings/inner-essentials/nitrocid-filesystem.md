...
<summary>UNIX permissions</summary>

Filesystem APIs provide you a way to manage UNIX permissions. For manipulating with file permissions, use the below functions:

* `GetUnixFileMode()`
* `SetUnixFileMode()`

### <mark style="color:$primary;">Management tools</mark>

The `UnixPermissionManager` class contains functions that allow you to perform calculation operations on Unix permissions, such as calculating a single `chmod` value to get permission flags, which can be determined with the `UnixPermissionType` enumeration, or getting a `chmod` value from `UnixPermissionType`.

You can also get the `UnixPermissionType` value from a string representation, which consists of:
...
However, to get full permission representation for user, group, and other, you'll need to use functions like `GetDescriptors()` to get an array of UNIX permission descriptors from either a set of `UnixFileMode` flags from .NET or a full `chmod` number (such as `755` or `644`), which contain permission descriptors and are editable.

Finally, you can convert a set of permission descriptors with special flags to a single `UnixFileMode` instance, and vice versa.

### <mark style="color:$primary;">Permission descriptors</mark>

`UnixPermissionDescriptor` is a class that can be created by a kernel mod. You can edit its properties, and you can calculate the single `chmod` number directly from the instance. It contains the following properties:

<table><thead><tr><th width="119.6666259765625">Property</th><th>Description</th></tr></thead><tbody><tr><td><code>Types</code></td><td>Unix permission types</td></tr><tr><td><code>Scope</code></td><td>Unix permission scope</td></tr></tbody></table>

### <mark style="color:$primary;">Permission enumerations</mark>

The `UnixPermissionScope` enumeration contains:

| Value   | Description                                                                                                         |
| ------- | ------------------------------------------------------------------------------------------------------------------- |
| `User`  | User permission scope (owner of the file as per chown)                                                              |
| `Group` | Group permission scope (group which owns the file as per chown)                                                     |
| `Other` | Other permission scope (user that doesn't belong to a specified group and is not an owner of the file as per chown) |

The `UnixPermissionSpecial` enumeration contains:
...
The `UnixPermissionType` enumeration contains:
